resource "aws_launch_configuration" "production_admin_api" {
  name          = "portl_production_admin_api_c5.large_${data.aws_ami.ecs_ami.id}"
...
resource "aws_autoscaling_group" "production_admin_api" {
  name                 = "portl_production_admin_api"
  force_delete         = true
  launch_configuration = "${aws_launch_configuration.production_admin_api.name}"
  ...
  tags = [
    ...
    {
      key                 = "Groups"
      value               = "auto,production,ecs"
      propagate_at_launch = true
    },
    {
      key                 = "Name"
      value               = "ecsN.prod.admin"
      propagate_at_launch = true
    },
    {
      key                 = "Project"
