...
resource "aws_cloudfront_distribution" "admin_ui" {
  aliases = ["${aws_acm_certificate.admin-ui_staging.domain_name}"]
...
s3_origin_config {
  ...
  "viewer_certificate" {
    acm_certificate_arn = "${aws_acm_certificate_validation.admin-ui_staging_validation.certificate_arn}"
...
resource "aws_cloudfront_origin_access_identity" "staging_media" {
  comment ="${title(var.app_name)} ${var.app_env} CloudFront Media"
}

resource "aws_cloudfront_distribution" "admin_ui_media" {
  aliases = ["${aws_acm_certificate.admin-ui-media_staging.domain_name}"]
  default_root_object = "index.html"
  enabled         = true
  is_ipv6_enabled = true
  ...
  "origin" {
    ...
    s3_origin_config {
      origin_access_identity = "${aws_cloudfront_origin_access_identity.staging_media.cloudfront_access_identity_path}"
    }
  }
  "default_cache_behavior" {
    allowed_methods = ["GET", "HEAD", "OPTIONS"]
  ...
  "viewer_certificate" {
    acm_certificate_arn = "${aws_acm_certificate_validation.admin-ui-media_staging_validation.certificate_arn}"
    ssl_support_method = "sni-only"
    minimum_protocol_version = "TLSv1.1_2016"
  }
  tags {
