data "template_file" "admin_ui_s3_policy" {
  ...
  vars {
    ...
    CF_access_identity_arn = "${aws_cloudfront_origin_access_identity.staging.iam_arn}"
  }
}
...
resource "aws_s3_bucket" "admin_ui" {
  bucket = "admin.staging.portl.com"
  acl = "private"
...
data "template_file" "admin_ui_s3_media_policy" {
  ...
  vars {
    ...
    CF_access_identity_arn = "${aws_cloudfront_origin_access_identity.staging_media.iam_arn}"
  }
}
...
resource "aws_s3_bucket" "admin_ui_media" {
  ...
  cors_rule {
    allowed_headers = ["*"]
    allowed_methods = ["PUT"]
    allowed_origins = ["https://${aws_route53_record.admin-ui_staging_portl_com_public.fqdn}"]
  }
